...
void listenBluetooth()
{
  ...
  {  
    ...
    // Leer un byte y colocarlo en 
      rxAvailable=bluetooth.available();
      modulSelect=bluetooth.read();
      rxData1= bluetooth.readStringUntil('.');       
...

      {
          ...
          case 's':
            rxData1.toCharArray(RFData,5);
            radio.write( RFData, 5 );
          break;
          default:
...
void help()
{
      switch(rxData[0])
   {
              case 'h': //help
               mode=0;
               bluetooth.println("Welcome to the Help Menu. These are the things the dittoV1 can do, for now:\n ");
               bluetooth.println(" - Start position: Press 's.' to bring the ditto to the neutral position.\n ");
               bluetooth.println(" - Run mode: Press 'r.' to try the experimental running feature.\n ");
               bluetooth.println(" - Manual mode: Press 'm.' to control manually each motor.\n ");
               bluetooth.println(" - Press 'e.' at anytime to get out the last mode selected.\n ");
               bluetooth.println(" - Press 'h.' at anytime to get information about the mode you are in.\n ");
               rxData[0]=1;
                break;
              case 's': //start
               mode=1;   
               bluetooth.println("Motors set to the neutral position.\n ");
          
                break;
              case 'r': //run
               mode=2; 
               bluetooth.println("Ditto is evolving into an Onix!\n ");
               bluetooth.println(" - What direction shall he take?\n ");
                break;
              case 'm': //manual
               mode=3; 
               bluetooth.println("Ditto is now in manual mode. Change your terminal to keyboard mode.\n ");
                break;
              case 'e': //Esc
               mode=4; 
                break;
...
void snake()
{
    while (mode==2)
    {
        ...
        switch(rxData[0])
         {
              case 'h': //help
                 bluetooth.println(" DittoV1 is in the run mode. Press 'e.' to return to the menu.\n ");
                 rxData[0]=1;
                 break;
              case 'e': //Esc
                 mode=0; 
...
listenBluetooth();
  ...
  switch(rxData[0])
   {
              case 'h': //help
               bluetooth.println(" DittoV1 is in the manual mode. Send your command in this format: 'M090.'\n ");
               bluetooth.println(" Press 'e.' to return to the menu.\n ");
               rxData[0]=1;
                break;
              case 'e': //Esc
               mode=0; 
